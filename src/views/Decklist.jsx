...
const initialLoading = (
  <span className="loading-content">Loading Cards...</span>
);
...
const Deck = ({ ...props }) => {
  ...
  useEffect(() => {
    if (decksInfo.id != null) chartColors();
    // eslint-disable-next-line
  }, [colors]);

  useEffect(() => {
    if (decksInfo.id != null) chartCMC();
    // eslint-disable-next-line
  }, [convertedManaCost]);
  ...
  const renderUniqueTypes = (types) =>
    types.map((type) => type[0] + ": " + type[1] + "\n");

  const loadDecks = async () => {
    let deckID = props.id;
    if (!deckID) deckID = window.location.search.substring(4);

    try {
      const res = await fetch(
        "/api/decks?type=one&deckID=" + deckID + "&authID=" + userID
      );
      if (res.status !== 200) {
        errorHandling(res);
        return;
  ...
  const retrieveStats = () => {
    //TODO: Combine action w/other mapping classes?
    ...
    const CMC = deck
      .map((card) => card.convertedManaCost)
      .reduce((sum, current) => sum + current);
    const AverageCMC = (CMC / quantity).toFixed(1);

    let types = mapExistingTypes(
      deck
        .map((card) => card.types)
        .reduce((sum, current) => sum + current)
        .toString()
        ...
        <Col md="4">
          ...
          >
            <span>
              ...
              <span id="uniqueQuantity">{existingTypes.length}</span>
            </span>
          </OverlayTrigger>
        </Col>
      </Row>
    ...
    while ((val = valueIterator.next().value) !== undefined) {
      ...
      let mapped = val.map((card, index) => {
        ...
        if (cardName.includes("//")) {
          //If two faced card, separate the card names for overlay
        ...
        return (
          <Row className="decklist-card" key={"card" + index + key}>
            ...
            >
              <span className="ellipsis default">
                {quantity}x {cardName}
              </span>
            </OverlayTrigger>
          </Row>
  ...
  const chartColors = () => {
    ...
    while ((val = valueIterator.next().value) !== undefined) {
      const key = keysIterator.next().value;
      if (val <= 0)
        //Ensure we only store active colors (at least 1 card of the color)
        continue;
      chart.data.push(val);
  ...
  return (
    <Container id="selection">
      <Row
        id="title"
        className="h-100 justify-content-around align-items-center"
      >
      ...
      >
        ...
        <Col md="3" className="opacity-layer pb-2">
          ...
          <Row className="justify-content-center">
            <MTGCard commander={true} title={commander} />
          </Row>
        </Col>
      </Row>
